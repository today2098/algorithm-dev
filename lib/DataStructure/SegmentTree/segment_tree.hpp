...
auto range_minimum_query(int n) {
    assert(n >= 0);
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return std::min(lhs, rhs); };
    constexpr auto e = []() -> S { return std::numeric_limits<S>::max(); };
    return SegmentTree<S, op, e>(n);
}

template <typename S>
auto range_minimum_query(const std::vector<S> &v) {
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return std::min(lhs, rhs); };
    constexpr auto e = []() -> S { return std::numeric_limits<S>::max(); };
...
auto range_maximum_query(int n) {
    assert(n >= 0);
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return std::max(lhs, rhs); };
    constexpr auto e = []() -> S { return std::numeric_limits<S>::lowest(); };
    return SegmentTree<S, op, e>(n);
}

template <typename S>
auto range_maximum_query(const std::vector<S> &v) {
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return std::max(lhs, rhs); };
    constexpr auto e = []() -> S { return std::numeric_limits<S>::lowest(); };
    return SegmentTree<S, op, e>(v);
}
...
auto range_sum_query(int n) {
    assert(n >= 0);
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return lhs + rhs; };
    constexpr auto e = []() -> S { return S(); };
    return SegmentTree<S, op, e>(n);
}

template <typename S>
auto range_sum_query(const std::vector<S> &v) {
    constexpr auto op = [](const S &lhs, const S &rhs) -> S { return lhs + rhs; };
    constexpr auto e = []() -> S { return S(); };
    return SegmentTree<S, op, e>(v);
}
